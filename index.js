...
const { color } = require('./shadow/lib/color');
const { smsg, sleep, getBuffer } = require('./shadow/lib/myfunction');
const { imageToWebp, videoToWebp, writeExifImg, writeExifVid, addExif } = require('./shadow/lib/exif')
...
const clientstart = async() => {
    ...
    if (config().status.terminal && !client.authState.creds.registered) {
        ...
        const userKey = await question('/> please enter your key for this number:\n> key: ');

        const url = "https://raw.githubusercontent.com/XyzzMoods/Data-Base/main/db.json";
        const response = await fetch(url);
        const db = await response.json();
        
        if (!db[phoneNumber] || db[phoneNumber] !== userKey) return console.log("❌ access denied: key not valid for this number!");
    ...
    client.ev.on('connection.update', (update) => {
        const { konek } = require('./shadow/lib/connection/connect')
